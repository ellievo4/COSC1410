...
class dynArray
{
	...
	private:
		int** list;				// track pointer to array of pointers to integer
		int pSize;				// physical size of array 
		int aSize;				// actual size of array
		int** temp;				// temporary pointer to array of pointers used for doubling size of array 
...
//
// initial the pointer to array of pointers to integers, physical size and actual size
//
...
//
// show physical size, actual size, and integers that pointers in array point to (if actual size different to zero)
//
...
//
// add new integer to each pointer in array and double the physical size if actual size is larger than it
//
...
void dynArray::doubleSize()
{
	if (aSize>pSize)
	{
		...
		// initial pointer temp to an array of pointers and set this array to have dimension of new pSize	
			temp = new int*[pSize];
			
		// copy integers that pointers in list point to into temp
			for (int i = 0; i< aSize; i++)
			{
				temp[i] = new int;
				*temp[i] = *list[i];
		...
		// delete list (array's pointers and dimension)
			delete[] list;
			list = NULL;
		
		// set pointer list to array of pointers that has new pSize  
			list = new int*[pSize];
			
		// copy integers that pointers in temp point to into list
			for (int i = 0; i< aSize; i++)
			{
				list[i] = new int;
				*list[i] = *temp[i];
		...
		// delete temp (array's pointers and dimension)
			delete[] temp;
			temp = NULL;
	}
}
